...
import JsonLd, { blogPostSchema, breadcrumbSchema } from "@/components/app/JsonLd";
...
const FALLBACK_POSTS = {
  "seo-mistakes-small-businesses-make": {
    ...
    date: "June 28, 2025", readTime: "6 min read", category: "SEO",
...
**The good news?** All of these are fixable. Most take less than an hour. Run a free audit on Goodly to see exactly which issues your site has — and get a step-by-step plan to fix them.`,
  ...
  "how-to-rank-number-one-google": {
    ...
    date: "June 25, 2025", readTime: "8 min read", category: "SEO",
    image: "https://images.unsplash.com/photo-1571171637578-41bc2dd41cd2?w=1200",
...
The businesses that win are the ones that keep improving. Run regular audits, fix issues as they appear, and watch your rankings climb.`,
  ...
  "instagram-for-small-business": {
    ...
    date: "June 22, 2025", readTime: "7 min read", category: "Social Media",
...
Instagram is a powerful tool for small businesses. Start with these basics, stay consistent, and watch your community grow.`,
  ...
  "google-business-profile-guide": {
    ...
    date: "June 20, 2025", readTime: "5 min read", category: "Local SEO",
...
A well-optimized Google Business Profile is the easiest way to get more local customers. It's free, it takes an afternoon to set up, and it pays dividends for years.`,
  ...
  "ai-visibility-small-business": {
    ...
    date: "June 18, 2025", readTime: "6 min read", category: "AI",
...
AI search is only going to grow. The businesses that optimize for it now will have a head start.`,
  ...
  "page-speed-seo-ranking": {
    ...
    date: "June 15, 2025", readTime: "5 min read", category: "SEO",
...
export default function BlogPost() {
      ...
      try {
        ...
        if (!cancelled && data) {
          setPost({
            ...
            image: data.image_url || "https://images.unsplash.com/photo-1432888498266-38ffec3eaf0a?w=1200",
          });
        }
      } catch (e) {
        // Use fallback
        if (!cancelled) setPost(FALLBACK_POSTS[slug] || null);
      } finally {
  ...
  usePageMeta({ title: post?.title, description: (post?.excerpt || post?.content || "").substring(0, 160) });
  ...
  useEffect(() => {
    ...
    script.textContent = JSON.stringify({
      ...
      "publisher": { "@type": "Organization", "name": "Goodly", "logo": { "@type": "ImageObject", "url": FRONTEND_URL + "/favicon.svg" } }
    });
    ...
    return (
      <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center">
        <div className="animate-pulse space-y-4 w-full max-w-3xl px-6">
          <div className="h-4 bg-[#F3F0E9] rounded w-1/4" />
          <div className="h-8 bg-[#F3F0E9] rounded w-3/4" />
          <div className="h-64 bg-[#F3F0E9] rounded-2xl" />
          <div className="h-4 bg-[#F3F0E9] rounded w-full" />
      ...
      <article className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mt-12 bg-gradient-to-r from-[#2D3E32] to-[#4A5F4F] rounded-2xl p-8 text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/audit" className="inline-block bg-[#E07A5F] hover:bg-[#D06A4F] text-white rounded-full px-6 py-3 font-medium">
              Run free audit <ArrowRight size={16} className="inline ml-1" />
            </Link>
            <Link to="/register" className="inline-block bg-white/10 hover:bg-white/20 text-white rounded-full px-6 py-3 font-medium">
              Create free account
            </Link>
          </div>
        </div>
...
function renderContent(text) {
  // Escape HTML to prevent XSS from blog content
  const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
  return text
    .split("\n\n")
    .map((block, i) => {
      if (block.startsWith("**") && block.includes("**")) {
        const cleaned = esc(block.replace(/\*\*/g, ""));
        return `<h2 class="font-display font-bold text-xl text-[#1A201A] mt-8 mb-3">${cleaned}</h2>`;
      }
      if (block.startsWith("- ")) {
        const items = block.split("\n").filter(l => l.startsWith("- "));
        return `<ul class="space-y-2 my-4">${items.map(item => `<li class="flex items-start gap-2 text-[#5C685C]"><span class="text-[#81B29A] mt-1">•</span> ${esc(item.replace("- ", ""))}</li>`).join("")}</ul>`;
      }
      if (block.startsWith("1. ")) {
        const items = block.split("\n").filter(l => /^\d+\./.test(l));
        return `<ol class="space-y-3 my-4 list-decimal list-inside text-[#5C685C]">${items.map(item => `<li class="pl-1"><span class="font-medium text-[#1A201A]">${esc(item.replace(/^\d+\.\s*/, ""))}</span></li>`).join("")}</ol>`;
      }
      if (block === "---") return '<hr class="my-8 border-[#E5E0D8]" />';
      return `<p class="text-[#5C685C] leading-relaxed mb-4">${esc(block)}</p>`;
    })
    .join("");
}
