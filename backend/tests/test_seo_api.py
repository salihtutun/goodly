...
class TestAuth:
    ...
    def test_login_bad_password(self, session):
        r = session.post(f"{API}/auth/login", json={"email": DEMO_EMAIL, "password": "***"}, timeout=20)
    ...
    def test_register_new_user(self):
        ...
        r = requests.post(f"{API}/auth/register", json={"email": email, "password": "***", "name": "Test User"}, timeout=20)
        assert r.status_code == 200, r.text
        data = r.json()
    ...
    def test_register_duplicate(self):
        r = requests.post(f"{API}/auth/register", json={"email": DEMO_EMAIL, "password": "***"}, timeout=20)
        assert r.status_code == 400

    def test_register_short_password(self):
        email = f"TEST_short_{uuid.uuid4().hex[:6]}@example.com"
        r = requests.post(f"{API}/auth/register", json={"email": email, "password": "***"}, timeout=20)
